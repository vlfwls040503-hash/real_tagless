...
def create_simulation():
    ...
    # 에스컬레이터 진입 waypoint (exit 대신 — JuPedSim 자동 remove 우회)
    # Python 로직이 이 waypoint 근처 에이전트를 직접 관리 (소프트웨어 큐)
    escalator_wp_upper = sim.add_waypoint_stage(
        ((EXITS[0]["x_start"] + EXITS[0]["x_end"]) / 2, EXITS[0]["y"] - 0.5),
        0.3
    )
    escalator_wp_lower = sim.add_waypoint_stage(
        ((EXITS[1]["x_start"] + EXITS[1]["x_end"]) / 2, EXITS[1]["y"] + 0.5),
        0.3
    )
...
def run_simulation():
    ...
    # ── 에스컬레이터 소프트웨어 큐 (게이트 off-on 로직 재활용) ──
    # 실제 에스컬레이터(30 m/min, 1.0m 폭) 처리율 ≈ 1.17 ped/s (Cheung & Lam 2002)
    ESCALATOR_SERVICE_TIME = 0.85
    # Capture zone: escalator_wp 주변 1m × 2m. 에이전트가 이 영역에 들어오면
    # Python이 busy 상태가 아닐 때 1명 intercept. busy 중에는 그대로 두어
    # wp 근처에 자연 누적 (CFSM 혼잡 형성).
    CAPTURE_UPPER = (27.0, 28.0, 22.5, 24.0)  # wp_upper(27.5,23.5) 주변
    CAPTURE_LOWER = (27.0, 28.0, 3.0, 4.5)    # wp_lower(27.5,3.5) 주변
    escalator_state = {
    ...
    ESC_WP_UPPER = (27.5, 23.5)
    ESC_WP_LOWER = (27.5, 3.5)
    ...
    for step in range(total_steps):
        ...
        # ── 에스컬레이터 소프트웨어 큐: capture zone intercept (iterate 전) ──
        # busy 아닐 때만 1명 intercept → 서비스 타이머 시작.
        # busy 중에는 intercept 안 함 → 에이전트는 자연 혼잡으로 대기 또는
        # exit polygon 도달 시 JuPedSim이 자동 제거 (통계는 기록됨).
        for side, zone in [("upper", CAPTURE_UPPER), ("lower", CAPTURE_LOWER)]:
            ...
            # busy 아닐 때 capture zone 안 에이전트 1명 intercept
            # (iterator 무효화 방지를 위해 후보 먼저 수집)
            candidate_aid = None
            for agent in sim.agents():
                px, py = agent.position
                if xmin <= px <= xmax and ymin <= py <= ymax:
                    candidate_aid = agent.id
                    break
